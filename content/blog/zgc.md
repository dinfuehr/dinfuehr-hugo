...
Having metadata information in heap references does make dereferencing more expensive, since the address needs to be masked to get the *real* address (without metainformation) like in this pseudo code:

```c++
intptr_t real_address(intptr_t address) {
...
ZGC employs a nice trick to avoid this: When reading from memory either the bit for `marked0`, `marked1` or `remapped` is set.
...
Physical memory can be expanded up to the maximum heap size (set with `-Xmx` for the JVM), so this is usually much less than the 4 TB of virtual memory.
...
During the marking-phase the load-barrier in application-threads pushes unmarked references into a thread-local marking buffer.
...
### The Load-Barrier
ZGC needs a so called load-barrier (sometimes referred to as read-barrier) when reading a reference from the heap.
...
Synchronization isn't necessary here since `ZAddressBadMask` only gets updated when the world is stopped, while no application thread is running.
